    ...
    class Program
    {
       ...
       // private static Orbwalking.Orbwalker OW;
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            // Print
            Game.PrintChat(String.Format("<font color='#08F5F8'>TIEN -</font> <font color='#FFFFFF'>{0} Loaded!</font>", champName));
        }
        ...
        private static bool isCC()
        {
            if (ObjectManager.Player.IsDead || ObjectManager.Player.IsImmovable || ObjectManager.Player.IsRooted || ObjectManager.Player.IsZombie || ObjectManager.Player.IsPacified || ObjectManager.Player.IsStunned)
            {
                return true;
            }
            return false;
        ...
        private static void OnCombo()
        {
            ...
            bool useE = comboMenu.Item("comboUseE").GetValue<bool>() && E.IsReady();
                ...
                if (useW && !Q.IsReady() && !E.IsReady() && EnemyNearObject(player, 850) > 0)
                {
                    W.Cast(player, true);
                }
        ...
        private static void AutoUlt()
        {
            if (menu.Item("ultUseR").GetValue<bool>())
            {
                foreach (Obj_AI_Hero AChamp in ObjectManager.Get<Obj_AI_Hero>())
                    ...
                    if (EnemyNearObject(AChamp, 700) >= 1)
                                {
        ...
        private static int EnemyNearObject(GameObject target, int range)
        {
            int numEnemyNear = 0;
