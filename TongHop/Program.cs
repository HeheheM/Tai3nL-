...
namespace TongHop
{
    class Program
    {
        ...
        public static Items.Item FrostQueen;
        public static Items.Item FaceMountain;
        ...
        private static void AntiStunFriend()
        {
            try
            {
                Obj_AI_Hero Hero = ObjectManager.Get<Obj_AI_Hero>().First(hero => hero.IsAlly && !hero.IsDead && (hero.HasBuffOfType(BuffType.Snare) || hero.HasBuffOfType(BuffType.Stun)) && hero.Distance(Tuong) <= Mikael.Range && Mikael.IsReady());
                if (Hero != null)
                 {   Mikael.Cast(Hero);
                     return;
                 }
        ...
        private static void SItem()
        {
                if (Menu.Item("FrostQueen").GetValue<bool>())
                {
                    if (FrostQueen.IsReady())
                    {
                        Obj_AI_Hero Hero = ObjectManager.Get<Obj_AI_Hero>().First(hero => hero.IsEnemy && !hero.IsDead && hero.Health * 100 / hero.MaxHealth <= Menu.Item("FrostQueenPer").GetValue<Slider>().Value && hero.Distance(Tuong) <= FrostQueen.Range);
                        if (Hero != null) 
                        {
                            FrostQueen.Cast(Hero);
                            return;
                        }
                    }
                }
                ...
                if (Menu.Item("FaceMountain").GetValue<bool>())
                {
                    if (FaceMountain.IsReady())
                    {
                        Obj_AI_Hero Hero = ObjectManager.Get<Obj_AI_Hero>().First(hero => hero.IsAlly && !hero.IsDead && (hero.Health * 100 / hero.MaxHealth <= Menu.Item("FaceMountainPer").GetValue<Slider>().Value) && hero.Distance(Tuong) <= FaceMountain.Range);
                        //Obj_AI_Hero Hero = GetLowestAlly(FaceMountain.Range);
                        if (Hero != null && Hero.Health * 100 / Hero.MaxHealth <= Menu.Item("FaceMountainPer").GetValue<Slider>().Value)
                        {
                            int num = EnemyNearObject(Hero, 700);
                            if (num >= 1)
                            {
                                FaceMountain.Cast(Hero);
                                return;
                            }
                        ...
                        {
                            Game.PrintChat("No Heroes found");
                        }
                        //if (Tuong.Health * 100 / Tuong.MaxHealth <= Menu.Item("FaceMountainPer").GetValue<Slider>().Value)
                        //{
                        //    FaceMountain.Cast(Tuong);
                        //}
                    }
                }
        }

        private static int EnemyNearObject(GameObject target, float range)
        {
            int numEnemyNear = 0;
            foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>())
            {
                if (hero.IsValid && hero.IsVisible && !hero.IsInvulnerable && !hero.IsDead && hero.IsEnemy && Vector3.Distance(target.Position, hero.Position) <= range)
                    numEnemyNear = numEnemyNear + 1;

            }
            return numEnemyNear;
        }

        private static Obj_AI_Hero GetLowestAlly(float range)
        {
            Obj_AI_Hero lowestChamp = new Obj_AI_Hero();
            lowestChamp = null;
            foreach (Obj_AI_Hero Champ in ObjectManager.Get<Obj_AI_Hero>())
                if ((!Champ.IsDead) && (Champ.IsAlly) && Vector3.Distance(ObjectManager.Player.ServerPosition, Champ.ServerPosition) <= range)
                {
                    if (lowestChamp == null || Champ.Health < lowestChamp.Health)
                    {
                        lowestChamp = Champ;
                    }
                }
